...
AI 不是这样。同样的问题，AI 今天给一个答案，明天给另一个，都不算错，但就是不一样。某个智能客服项目里，用户问「我的快递到哪了」，第一次 AI 直接告诉他在哪个转运中心；两分钟后同一个用户又问了一遍，AI 这次要求他提供订单号。两次回答都有道理，但用户体验是混乱的。
...
某个做内容审核的团队曾经跟我说过他们的经历。最早用关键词库，三千多个敏感词，人工维护，准确率 72%。换成 AI 模型之后准确率到了 94%，因为 AI 在理解语义，不是在匹配字符串。但代价是，你再也没法打开一个 Excel 就能看到所有判断规则。某天 AI 突然把一条正常内容标记为违规，你想知道为什么——答案是「模型就是这么判断的」。
...
某个医疗写作项目，帮医生写论文初稿。上线时医生对 AI 生成内容的采纳率是 62%，三个月后没做任何改版，采纳率涨到了 78%。系统在收集医生的反馈数据，模型一直在自动优化。
...
我现在觉得，从传统软件思维切换到 AI 产品思维，最难的一步不是学技术，是放弃「系统应该可以被完全掌控」这个预设。

写 PRD 的时候，你不再能把每个输出都定义清楚，因为输出本身就是不确定的。测试的时候，你不再能用「功能是否正常运行」作为唯一标准，因为 AI 的输出没有唯一正确答案。迭代的时候，你不再能假设系统状态是稳定的，因为模型随时在变。

这不是让产品经理束手无策，而是让产品经理的工作变了性质。从「定义系统应该怎么运行」，变成「定义系统在各种情况下应该如何表现，包括它表现不好的时候」。
