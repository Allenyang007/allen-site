...
我参与过某个 B 端项目，做企业知识库问答。团队里有两个 PM 分别负责不同模块。一个花了两周理解了大模型的基本原理，另一个觉得"这些是技术的事"。
...
我的判断是：你不需要懂怎么训练模型，但你必须懂模型的行为边界。不是因为这样更"全面"或者更"高级"，而是因为在 AI 产品里，如果你不懂这些，你做的每一个产品决策都可能建立在错误的假设上——只是你自己不知道而已。

留一个问题：如果一个 PM 可以完全不懂大模型原理就能做好 AI 产品，那 AI 产品经理和传统产品经理的区别到底在哪里？或者说，这个岗位存在的理由是什么？
