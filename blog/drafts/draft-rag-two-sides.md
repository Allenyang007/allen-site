...
去年在某个医学 AI 写作项目里，我第一次认真用 RAG 解决生产问题。团队需要让模型引用真实的医学文献来写综述，而不是一本正经地编造 DOI 号和论文标题——没错，大模型真的会这么干，而且编得像模像样，不仔细核查根本看不出来。
...
第二个是幻觉。这个问题在专业领域尤其要命。我见过模型编造建筑规范条款号的情况，格式完美、逻辑自洽，就是这个条款压根不存在。如果下游有人拿着这个"规范"去做设计决策，后果不堪设想。RAG 让模型的回答有了依据——知识库里有就说，没有就闭嘴，至少方向对了。
...
但我想对正在做 RAG 项目的人说一句：不要低估工程复杂度。RAG 的论文和教程会让你觉得这事儿很简单——切块、向量化、检索、生成，四步搞定。实际落地的时候，你会发现 80% 的时间花在数据清洗、分块调优、评估迭代这些"不性感"的环节上。
